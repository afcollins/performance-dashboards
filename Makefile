...
SYNCER_IMG_TAG ?= quay.io/afcollins/dittybopper-syncer:go-sdk-f514445-amd64
...
build-syncer-image: build
	podman build --platform=${PLATFORM} -f Dockerfile --tag=${SYNCER_IMG_TAG} .

push-syncer-image:
	podman push ${SYNCER_IMG_TAG} ${SYNCER_IMG_TAG}
